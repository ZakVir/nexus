...
// src/manager.ts
var PLUGINS_DIR = join(homedir(), ".nexus", "plugins");

class PluginManager {
  plugins = new Map;
  slotHandlers = new Map;
  loadAll() {
  ...
  loadPlugin(pluginDir, name) {
    const manifestPath = join(pluginDir, "index.js");
    const manifest = __require(manifestPath);
    ...
    for (const [slot, handler] of Object.entries(manifest.slots)) {
      if (typeof handler === "function") {
        this.registerSlotHandler(slot, handler);
      }
    }
  }
