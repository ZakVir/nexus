...
var VERSION = "0.1.0";
var NEXUS_DIR = join(homedir(), ".nexus");
var CONFIG_PATH = join(NEXUS_DIR, "config.json");
var KEYS_PATH = join(NEXUS_DIR, "keys.json");
...
function isHeadlessMode() {
  if (values.print || values.json || values.pipe || values.oneshot)
    return true;
...
function ensureConfig() {
  ...
  if (!existsSync(KEYS_PATH)) {
    writeFileSync(KEYS_PATH, "{}", { mode: 384 });
  }
}
function loadConfig() {
  ensureConfig();
...
async function runHeadless(config) {
  ...
  const model = values.model || "unknown";
  const mode = values.mode || config.mode || "single";
  const project = config.project_name || "Nexus";
...
Setup complete! Press Enter to launch...`);
  ...
  const homeOutput = renderHomeScreen({
    projectName: config.project_name || "Nexus",
    version: VERSION,
    currentModel: values.model || "default",
